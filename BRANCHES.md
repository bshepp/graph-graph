...
| **`d(p)` multi-bump fine structure — mechanism hunt (round 2)** | overnight pre-registered tests 2026-08-12 (spec a134c15): N-invariance PASSED at 16x (corr +0.95..0.96, period shift ≤2%) but the log-periodic extrapolation was REFUTED (no third cycle below p=0.02) and the protection-hierarchy onset story unsupported (cluster onsets sit in the trough, not at peaks) | (was "log-periodic fine structure") | a real, deterministic, density-intrinsic multi-bump structure with NO surviving mechanism hypothesis — genuinely open. Next mechanism candidates to formulate before testing: (a) competition between ring-regime and mesh-regime ball growth inside the fixed fit window; (b) discrete convergence-depth bands of prune_to_convergence; (c) something in the WS construction's shortcut-overlap statistics. Formulate → pre-register → test, in that order | low-medium |
