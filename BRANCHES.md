...
| **`d(p)` log-periodic fine structure — mechanism hunt** | dense-grid follow-up 2026-08-11 (jaga): structure CONFIRMED real — perm-p 0.0005 at n=30, window-invariant (r=+0.96..0.99 across max_radius 8/10/12), seed-split r=+0.83, period ~1.6 ln p | (was the scan's "low-stakes curiosity") | first genuine log-periodic structure in the sandbox; mechanism open. Concrete next test: histogram surviving-shortcut cluster sizes vs p — protection-hierarchy hypothesis predicts cluster-generation onsets at the d(p) peaks (~0.02/0.10/0.36). Ties directly into the stage-1 throat protection-core mechanism | low-medium |
