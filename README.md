...
| `async_censorship.py` | Re-runs the banked shortcut-censorship result (FINDINGS.md §2) under **asynchronous** Poisson-clock updates (de-toying step 4 — the first *physics* checkpoint). Two pre-committed gates: pure-`prune` reproduces the threshold/advantage-blind P1 in distribution (the instrument gate + control), and the `triadic`+`prune` race tests whether P2's self-stabilization survives emergent time. `--validate` result: **P1 schedule-invariant, P2 survives but ~2× attenuated** vs synchronous |
