...
| `async_engine.py` | Asynchronous event-driven update engine (de-toying step 2): Poisson-clock sequential reference plus conflict-free batched execution. `--validate` proves batches are order-independent, with constructed negative controls, and checks batched-vs-sequential distributions |
| `causal_sets.py` | Causal-set dimension estimators (Myrheim–Meyer ordering fraction, midpoint scaling) calibrated against Poisson sprinklings into flat Minkowski space. `--validate` is the step-1 gate for the causal-DAG upgrade — no model dynamics, instruments only |
| `causal_dag.py` | Records the causal DAG of async update events (de-toying step 3) and estimates its causal-set dimension. `--validate` runs the static-graph calibration and its **key negative**: the event DAG is not manifold-like, so `d_causal != d_H+1` — with a fixed-speed-cone positive control proving the tool works and the model, not the instrument, reads low |
| `async_censorship.py` | Re-runs the banked shortcut-censorship result (FINDINGS.md §2) under **asynchronous** Poisson-clock updates (de-toying step 4 — the first *physics* checkpoint). Two pre-committed gates: pure-`prune` reproduces the threshold/advantage-blind P1 in distribution (the instrument gate + control), and the `triadic`+`prune` race tests whether P2's self-stabilization survives emergent time. result: **P1 schedule-invariant, P2 survives essentially unchanged** (12-seed paired estimate via `async_censorship_paired.py`: no significant attenuation) |
| `throat_criticality.py` | Stage 1 of the critical-collapse program: is there a sharp threshold in wormhole-throat thickness (evaporation vs a permanent mutually-protected strand core)? Peeling fixed point + ensemble bisection + stochastic anchor + triadic rider, all pre-registered. Verdict: **local-motif onset, not a critical point** (onset core ≈2.6 strands at every capacity) — but the peeling fixed point predicts prune dynamics *exactly* (40/40), and triadic demolishes censor-proof cores. `--validate` has hand-built known-answer throats |
| `logperiodic_scan.py` | Hunts for discrete scale invariance (log-periodic "spacetime crystal" modulation, à la Choptuik DSS) in the sandbox's scaling relations. Pre-registered method: Lomb–Scargle + permutation null + Bonferroni, an artifact-discriminator ladder (cycle count, detrend orders, staircase control), and injection-recovery so nulls are bounded. Result: **global null** — no DSS above ~10% amplitude. `--validate` plants a synthetic crystal the pipeline must find |
...
# Walkers: who actually benefits from a portal?
python shortcut_walkers.py --validate          # check the horizon-free observable
...
fixed (see the module docstring for why).

The walkers driver reports two *horizon-free* quantities in matched units — the
classical stationary occupancy `π(target) = deg(target)/2|E|` and the exact CTQW
infinite-time average `P̄` — plus, as a labelled transient diagnostic only, the
peak transfer over `(0, --t-max-factor × d]`. Prefer `P̄`: peak gains shrink as
the window grows, so they are not a well-defined quantity (the driver flags
peaks that land at the end of the grid). `--generators adjacency laplacian` runs
both Hamiltonians on the same graphs as a degree-artifact cross-check. `P̄` costs
a dense eigendecomposition, so keep N to a few thousand. Results:
[FINDINGS.md](FINDINGS.md) → "Portal experiments."
